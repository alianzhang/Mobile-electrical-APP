define(function(require) {
	...
	Model.prototype.shareBtnClick = function(event){
		...
		*/
		if (!navigator.weixin) {//判断有没有微信插件
			return;
		}
		var weixin = navigator.weixin;	
		weixin.share({//调用微信里面的share方法
			message : {
				title : this.comp("goodsData").val("fTitle"),
				description : "",
				mediaTagName : "",
				thumb : location.origin + require.toUrl("./detail/img/icon.png"),//缩略图
				media : {//分享的媒体类型
					type : weixin.Type.WEBPAGE,
					webpageUrl : Utils.getShareUrl()//Utils.getShareUrl()可以把location.href给分享出去
				}
			},
	...
	Model.prototype.colorLiClick = function(event){
		...
		*/
		...
		if(colorData.getValue("fState",row)==0){
			colorData.setValue("fState","1",row);
		} else {
			colorData.setValue("fState","0",row);
		}
	};	
