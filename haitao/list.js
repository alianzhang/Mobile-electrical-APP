define(function(require) {
	...
	Model.prototype.sortingListClick = function(event) {
		...
		data.setValue("fState", "0", data.find([ "fState" ], [ "1" ], true, true, true)[0]);//先找到上一次状态为1的行改为0
		var row = data.getCurrentRow();
		data.setValue("fState", "1", row);//把当前选中的行改为1
		this.comp("sortingBtn").set("label", data.getValue("fName", row));//把页面显示的button的label改为当前选中行的名字
		this.comp("sortingPopOver").hide();//再把遮罩隐藏起来
	};
